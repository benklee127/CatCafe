...
@onready var decor_layer: Node2D = get_parent().get_node("DecorLayer")

func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		var tile := world_to_iso(event.position)
...
func world_to_iso(pos: Vector2) -> Vector2i:
	var gx := floor((pos.x / (tile_width * 0.5) + pos.y / (tile_height * 0.5)) * 0.5)
	var gy := floor((pos.y / (tile_height * 0.5) - pos.x / (tile_width * 0.5)) * 0.5)
	return Vector2i(int(gx), int(gy))

func iso_to_world(tile: Vector2i) -> Vector2:
	var x := (tile.x - tile.y) * (tile_width * 0.5)
	var y := (tile.x + tile.y) * (tile_height * 0.5)
...
func _draw() -> void:
	for child in decor_layer.get_children():
		if child is Node2D:
			draw_rect(Rect2(to_local(child.global_position) - Vector2(12, 12), Vector2(24, 24)), Color(0.62, 0.62, 0.62))
