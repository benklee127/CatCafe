...
func can_place_decor(anchor_grid: Vector2i, decor_id: String, rotation_deg: int) -> bool:
	...
	var rot: int = _normalize_rotation(rotation_deg)
	if not allowed.has(rot):
		return false

	var footprint: Array = _footprint_cells(anchor_grid, mesh_entry, rot)
...
func place_decor(anchor_grid: Vector2i, decor_id: String, rotation_deg: int) -> bool:
	...
	decor_instance.position = map_to_world(anchor_grid) + pivot
...
func _footprint_cells(anchor: Vector2i, mesh_entry: Dictionary, rot: int) -> Array:
	var out: Array = []
	for raw in mesh_entry.get("footprint", []):
		var arr: Array = raw
		if arr.size() < 2:
			continue
		var off := Vector2i(int(arr[0]), int(arr[1]))
		out.append(anchor + rotate_offset(off, rot))
	return out
	...
	for kind_key in slot_markers.keys():
		...
		for marker_raw in slot_markers[kind_key]:
			...
			if arr.size() < 2:
				continue
			var off := Vector2i(int(arr[0]), int(arr[1]))
			var cell: Vector2i = anchor + rotate_offset(off, rot)
			var world_pos: Vector3 = map_to_world(cell) + Vector3(0.15, 0.0, 0.15)
